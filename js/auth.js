...
const DioAuth = (() => {
  ...
  function register(name, email, password, confirm, phone = '') {
    ...
    const result = DioDB.registerUser({ name: displayName, email, password, phone: normalizePhone(phone) });
  ...
  function loginByPhone(phone, password) {
    ...
    if (normalized.length < 9) return { ok: false, error: 'Số điện thoại không hợp lệ' };
    const user = DioDB.getUserByPhone(normalized);
    if (!user) return { ok: false, error: 'Số điện thoại chưa đăng ký' };
    if (user.isBanned) return { ok: false, error: 'Tài khoản đã bị khóa' };
    const hash = DioDB.hashPassword(password);
    if (user.password !== hash) return { ok: false, error: 'Mật khẩu không đúng' };
    DioDB.setSession(user.id);
    return { ok: true, user: DioDB.sanitizeUser(user) };
  }
  ...
  return { register, login, loginByPhone, logout, isLoggedIn, restore };
})();