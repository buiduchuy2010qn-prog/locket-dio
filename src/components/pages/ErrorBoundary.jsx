...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    // removeChild lệch DOM (boot shell / extension / snow race) — tự recover 1 lần
    const msg = String(error?.message || error || "");
    const isDomRace =
      /removeChild|NotFoundError|The node to be removed/i.test(msg);
    if (isDomRace) {
      const key = "hl_dom_recover";
      try {
        const n = Number(sessionStorage.getItem(key) || 0);
        if (n < 1) {
          sessionStorage.setItem(key, String(n + 1));
          // Clear SW cache stale rồi reload
          this.handleReload();
        }
      } catch {
        /* ignore */
        ...
        <div className="w-full h-[100dvh] bg-base-100 text-base-content flex flex-col items-center justify-center p-4 md:p-6">
          <div className="w-full max-w-4xl text-left">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-4">
              ...
              >
                <span className="inline-flex items-center gap-2">
                  {this.props.t("error_boundary.join_community")}
                </span>
              </a>
            </div>
          </div>
