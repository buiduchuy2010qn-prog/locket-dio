  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <div className={`${s} rounded-2xl gold-gradient flex items-center justify-center shadow-[var(--shadow-gold)] shrink-0`}>
        <span className="text-white font-extrabold tracking-tight">P</span>
      </div>
      {showWord && (
        <div className="leading-tight">
          <div className="font-extrabold text-lg tracking-tight text-slate-900 dark:text-white">
            Piclet<span className="gold-text"> Gold</span>
          </div>
          {size === 'lg' && <p className="text-xs text-slate-500">Khoảnh khắc của bạn bè</p>}
        </div>
      )}
    </div>
  )
