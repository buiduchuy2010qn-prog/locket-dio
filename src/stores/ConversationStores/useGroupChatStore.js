...
import { useMembersGroupStore } from "./useMembersGroupStore";
...
export const useGroupChatStore = create((set, get) => ({
  ...
  upsertGroup: (group) => {
    ...
    // Đảm bảo luôn có last_updated_at để sort/index hoạt động đúng
    map.set(group.id, {
      ...map.get(group.id),
      ...group,
      last_updated_at: group.last_updated_at || Date.now(),
    });
