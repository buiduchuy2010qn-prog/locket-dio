...
const { logInfo, logSuccess, logWarning, logTable } = require("../logEventUtils");
...
const calculateCustomCrop = (width, height, videoCropData) => {
  if (!videoCropData || 
      videoCropData.x === undefined || 
      videoCropData.y === undefined || 
      videoCropData.width === undefined || 
      videoCropData.height === undefined) {
    logWarning("calculateCustomCrop", "Invalid videoCropData, falling back to center crop");
    return calculateCenterCrop(width, height);
  }

  const xPercent = parseFloat(videoCropData.x);
  const yPercent = parseFloat(videoCropData.y);
  const wPercent = parseFloat(videoCropData.width);
  const hPercent = parseFloat(videoCropData.height);

  if (isNaN(xPercent) || isNaN(yPercent) || isNaN(wPercent) || isNaN(hPercent)) {
    logWarning("calculateCustomCrop", "NaN values in videoCropData, falling back to center crop");
    return calculateCenterCrop(width, height);
  }

  // Calculate pixel positions based on percentages (0 to 100)
  let cropX = Math.floor(width * (xPercent / 100));
  let cropY = Math.floor(height * (yPercent / 100));
  let cropW = Math.floor(width * (wPercent / 100));
  let cropH = Math.floor(height * (hPercent / 100));

  // Boundaries validation
  if (cropX < 0) cropX = 0;
  if (cropY < 0) cropY = 0;
  if (cropW <= 0) cropW = width;
  if (cropH <= 0) cropH = height;

  // Make sure coordinates are even numbers (requirement for yuv420p / libx264)
  ...
  logInfo("calculateCustomCrop", 
    `Custom crop: ${width}x${height} → ${cropW}x${cropH} at (${cropX}, ${cropY}) based on percentages x:${xPercent}%, y:${yPercent}%, w:${wPercent}%, h:${hPercent}%`
  );
...
const needsProcessing = (metadata, maxSizeMB, videoCropData = null) => {
  const { isSquare, isMp4, sizeMB } = metadata;

  if (videoCropData) {
    logInfo("needsProcessing", "Processing needed - custom crop requested");
    return { needed: true, reason: "custom_crop" };
  }
...
const cropToSquare = (inputPath, outputPath, metadata, videoCropData = null) => {
  return new Promise((resolve, reject) => {
    const { width, height } = metadata;
    const cropParams = videoCropData 
      ? calculateCustomCrop(width, height, videoCropData)
      : calculateCenterCrop(width, height);

    logInfo("cropToSquare", `Cropping to ${cropParams.width}x${cropParams.height} at x:${cropParams.x}, y:${cropParams.y}`);
...
const processVideoBuffer = ({ videoBuffer, filename = "temp", maxSizeMB = 5, videoCropData = null }) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // STEP 2: Check if processing needed
      const processingCheck = needsProcessing(metadata, maxSizeMB, videoCropData);
      ...
      if (!metadata.isSquare || videoCropData) {
        const croppedPath = path.join(tmpDir, `${timestamp}_${filename}_cropped.mp4`);
        await cropToSquare(currentPath, croppedPath, metadata, videoCropData);
