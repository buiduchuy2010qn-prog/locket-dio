...
const createBasePayload = ({
  ...
  const { recipients, audience, isStreaktoday, restoreStreakDate, streakData } =
    optionsData;
  ...
  } else if (!isStreaktoday) {
    logInfo("newStreakDate", "Nhận yêu cầu tạo chuỗi cho ngày", getYYYYMMDD());
    payload.update_streak_for_yyyymmdd = createIntValue(getYYYYMMDD());
  }

  if (streakData) {
    logInfo("streakData", "Gửi dữ liệu streak trong payload", streakData);
